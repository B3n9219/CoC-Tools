...
def update_cwl_sheet():
    ...
    if info_found:
        ...
        sheet.update_cell(f"{column_to_number(update_column)}1", entry_title ,config["cwl_sheet"])

        info_to_add = util.prepare_attack_info_to_add(players_in_sheet, players_in_clan, player_cwl_info, "Stars", "")
        util.add_attack_info_to_sheet(info_to_add, "Stars Earned", column_to_number(update_column), config["cwl_sheet"], 1)

        info_to_add = util.prepare_attack_info_to_add(players_in_sheet, players_in_clan, player_cwl_info, "AttacksUsed", "")
        util.add_attack_info_to_sheet(info_to_add, "Attacks Used", column_to_number(update_column+1), config["cwl_sheet"], 1)

        info_to_add = util.prepare_attack_info_to_add(players_in_sheet, players_in_clan, player_cwl_info, "AttacksAvailable", "")
        util.add_attack_info_to_sheet(info_to_add, "Attacks Available", column_to_number(update_column+2), config["cwl_sheet"], 1)

        sheet.update_cell(f"{column_to_number(update_column+3)}2", "Stars per Attack", config["cwl_sheet"])
        sheet.update_cell(f"{column_to_number(update_column+4)}2", "Attacks Missed", config["cwl_sheet"])